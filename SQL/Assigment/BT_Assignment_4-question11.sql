/*============================== CREATE DATABASE =======================================*/
/*======================================================================================*/
DROP DATABASE IF EXISTS BT_4;
CREATE DATABASE BT_4;
USE BT_4;
...
CREATE TABLE IF NOT EXISTS table11 (
	departposi_id		TINYINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    depart_posi			VARCHAR(100)
);

INSERT INTO 	table11 (depart_posi)
SELECT 			CONCAT(d.department_name, '_', p.position_name) AS depart_posi
FROM 			department d
CROSS JOIN 		position p;

ALTER TABLE `account`
ADD COLUMN depart_posi VARCHAR(100);

INSERT INTO 	`account` (depart_posi)
SELECT			 CONCAT(d.department_name, '_',p.position_name)
FROM 			`account` a
JOIN department d 	ON a.department_id = d.department_id
JOIN position p 	ON a.position_id = p.position_id;

SELECT 		t.depart_posi AS Phòngban_Vịtrí, COUNT(a.depart_posi) AS 'Số nhân viên'
FROM 		`account` a
RIGHT JOIN 	table11 t ON a.depart_posi = t.depart_posi
WHERE		t.depart_posi LIKE '%Dev'
GROUP BY 	t.depart_posi 

UNION
SELECT 		t.depart_posi AS Phòngban_Vịtrí, COUNT(a.depart_posi) AS 'Số nhân viên'
FROM 		`account` a
RIGHT JOIN	table11 t ON a.depart_posi = t.depart_posi
WHERE 		t.depart_posi LIKE '%Test'
GROUP BY 	t.depart_posi 
...
FROM 		`account` a
RIGHT JOIN 	table11 t ON a.depart_posi = t.depart_posi
WHERE		t.depart_posi LIKE '%Scrum Master'
GROUP BY 	t.depart_posi 
...
FROM 		`account` a
RIGHT JOIN 	table11 t ON a.depart_posi = t.depart_posi
WHERE		t.depart_posi LIKE '%PM'
GROUP BY 	t.depart_posi 
ORDER BY 	Phòngban_Vịtrí;
